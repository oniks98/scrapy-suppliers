...
Використання: 
  ...
  python scripts/ultra_clean_run.py eserver_retail --no-transform  (без трансформації)
"""
import sys
import os
import warnings
...
def run_transformation():
    ...
    # Шлях до скрипта трансформації (у тій же директорії)
    base_dir = Path(__file__).parent
    transform_script = base_dir / "transform_retail_to_prom.py"
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("❌ Використання: python scripts/ultra_clean_run.py <spider_name> [--no-transform]")
        print("📝 Приклад: python scripts/ultra_clean_run.py eserver_retail")
        print("📝 Без трансформації: python scripts/ultra_clean_run.py eserver_retail --no-transform")
        sys.exit(1)
